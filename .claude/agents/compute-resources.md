...
1. **Disk bandwidth** → [`kb/calibrations/disk-bandwidth.yaml`](../../kb/calibrations/disk-bandwidth.yaml).
   Currently `206.8 MB/s` on `D:`, measured 2026-08-12 → L3.1 budget
...
3. **Frame rate** → **from lens 2, or from a past acquisition's
   `drops` report.** Never from the user's intention. See L3.2 below.
...
L3.4 comes out optimistic.

### L3.2 `fps_provenance` — bias
...
2. Ceiling supplied, request fits → still warn. §C4's bottleneck was *not* the
   camera, so clearing L2.5 does not make the rate achieved.
3. Ceiling supplied, request exceeds it → `fail`-severity finding, margin < 1,
   ...
   realizability is lens 2's L2.5, and this lens does not seize a verdict it does
...
the fast one (500 fps full frame): a 2× error lands precisely where L3.1 binds,
...
hold the whole burst in memory, flush afterwards. This lifts L3.1 — nothing is
...
- **3 ↔ 2 (detection)**: the two directions are different. *Inbound*, lens 2's
  `max_fps` is the only defensible ceiling for L3.2 — never accept a bare
  requested rate. *Outbound*, when L3.1 or L3.4 fails, the fix (smaller ROI,
...
- **One finding is still owed to Lens 7.** The 2026-08-20 sweep found that
  ...
  wrong, so κ is wrong, and L7.2–L7.4 gates on that same `f_c`. Naming them is not
...
- **The 8-bit container is unconfirmed** on the real PVCAM/Kinetix adapter
  (L3.3). One reading of `core.getBytesPerPixel()` closes it.
