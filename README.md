...
| **30 deterministic gates** | G1–G32 less the vacant `G10` and `G20`, each classified `hard` / `bias` / `soft` by what its failure costs. **27 are implemented** — `G2`–`G4` carry a threshold and a default verdict in [04](docs/04-decision-engine.md) and appear in no Python file; `G10` and `G20` were both removed on 2026-09-09 and neither number is reused → [below](#two-more-axes-and-the-questions-neither-working-repo-asks) → [05 §2](docs/05-consensus-gate.md) |
...
```text
  ...
  +---------------------------------+---------------------------------+
                                    v
  +-------------------------------------------------------------------+
  |  COMMITTEE          8 lenses . 30 gates (G1-G32, no G10/G20)      |
  |                                                                   |
  ...
  |  discovery rung above. It does NOT touch the 30 gates, which sit  |
...
documents, 30 gates (G1–G32 less the vacant G10 and G20), 1,194 tests passing. The badge above reports
...
among the 30 gates blocks the verdict today, which is the only defensible
...
- **Write the stub, and stop.** A new `hardware/<device>/` with the description
  ...
  landing in it propagates into 30 gates.
...
| [04 Decision engine](docs/04-decision-engine.md) | Decision order, photon budget / SNR / sampling / timing formulas, the 30 gates |
